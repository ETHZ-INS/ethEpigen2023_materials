...
```{r}
d <- meltSignals(smb[c("Cuts", "Mono centers")])
...
```{r}
motifFootprint("atac.chr19.bam", motif, moi)
```
...
tss.sm <- signal2Matrix(tracks, granges(tss), w=5, extend=500)
tss.smb <- rescaleSignalMatrices(tss.sm, nf)
plotEnrichedHeatmaps(tss.smb, trim=0.95)
```
